...
seq_io::FileFormat check_that_all_files_have_the_same_format(
  ...
  for (int64_t i = 1; i < filenames.size(); i++) {
    ...
    if (f1.format != f2.format || f1.gzipped != f2.gzipped) {
      throw runtime_error("Error: not all input files have the same format (" +
                          filenames[0] + " vs " + filenames[i] + ")");
    }
  }
  return f1;
}
...
int build_main(int argc, char** argv) {
  ...
  for (string variant : variants) all_variants_string += "\n" + variant;

  options.add_options()(
      "i,in-file",
      "The input sequences as a FASTA or FASTQ file, possibly gzipped. If the "
      "file extension is .txt, the file is interpreted as a list of input "
      "files, one file on each line. All input files must be in the same "
      "format.",
      cxxopts::value<string>())("o,out-file",
                                "Output file for the constructed index.",
                                cxxopts::value<string>())(
      "k,kmer-length", "The k-mer length.", cxxopts::value<int64_t>())(
      "p,precalc-length",
      "Precalculate SBWT intervals of strings of this length. Speeds up query, "
      "but takes 4^(p+2) bytes of memory.",
      cxxopts::value<int64_t>()->default_value("8"))(
      "variant",
      "The SBWT variant to build. Available variants:" + all_variants_string,
      cxxopts::value<string>()->default_value("plain-matrix"))(
      "add-reverse-complements",
      "Also add the reverse complement of every k-mer to the index. Warning: "
      "this creates a temporary reverse-complemented duplicate of each input "
      "file before construction. Make sure that the directory at --temp-dir "
      "can handle this amount of data. If the input is gzipped, the duplicate "
      "will also be compressed, which might take a while.",
      cxxopts::value<bool>()->default_value("false"))(
      "no-streaming-support",
      "Save space by not building the streaming query support bit vector. This "
      "leads to slower queries.",
      cxxopts::value<bool>()->default_value("false"))(
      "t,n-threads", "Number of parallel threads.",
      cxxopts::value<int64_t>()->default_value("1"))(
      "a,min-abundance",
      "Discard all k-mers occurring fewer than this many times. By default we "
      "keep all k-mers. Note that we consider a k-mer distinct from its "
      "reverse complement.",
      cxxopts::value<int64_t>()->default_value("1"))(
      "b,max-abundance",
      "Discard all k-mers occurring more than this many times.",
      cxxopts::value<int64_t>()->default_value("1000000000"))(
      "m,ram-gigas",
      "RAM budget in gigabytes (not strictly enforced). Must be at least 2.",
      cxxopts::value<int64_t>()->default_value("2"))(
      "d,temp-dir", "Location for temporary files.",
  ...
  if (old_argc == 1 || opts.count("help")) {
    ...
    cerr << "Usage example: " << argv[0]
         << " -i example_data/coli3.fna -o index.sbwt -k 30" << endl;
    exit(1);
  }
  ...
  if (precalc_length > k) {
    write_log("Warning: precalc length " + to_string(precalc_length) +
                  " is longer than k = " + to_string(k),
              sbwt::LogLevel::MAJOR);
    write_log("Setting precalc length to " + to_string(k),
              sbwt::LogLevel::MAJOR);
    precalc_length = k;
  }

  seq_io::FileFormat fileformat =
      check_that_all_files_have_the_same_format(input_files);
  if (revcomps) {
    sbwt::write_log(
        "Creating a reverse-complemented version of each input file to " +
            temp_dir,
        sbwt::LogLevel::MAJOR);
    vector<string> new_files;
    for (int64_t i = 0; i < input_files.size(); i++) {
      new_files.push_back(sbwt::get_temp_file_manager().create_filename(
    ...
    if (fileformat.gzipped) {
      seq_io::create_reverse_complement_files<
          ...
          seq_io::Writer<seq_io::Buffered_ofstream<seq_io::zstr::ofstream>>>(
          input_files, new_files);
    } else {
      seq_io::create_reverse_complement_files<
          seq_io::Reader<seq_io::Buffered_ifstream<std::ifstream>>,
          seq_io::Writer<seq_io::Buffered_ofstream<std::ofstream>>>(input_files,
                                                                    new_files);
    }
  ...
  bytes_written += sbwt::serialize_string(
      variant, out.stream);  // Write variant string to file

  write_log("Build SBWT for " + to_string(matrixboss_plain.number_of_kmers()) +
                " distinct k-mers",
            sbwt::LogLevel::MAJOR);
  write_log("SBWT has " + to_string(matrixboss_plain.number_of_subsets()) +
                " subsets",
            sbwt::LogLevel::MAJOR);
  ...
  const sdsl::bit_vector& A_bits =
      matrixboss_plain.get_subset_rank_structure().A_bits;
  const sdsl::bit_vector& C_bits =
      matrixboss_plain.get_subset_rank_structure().C_bits;
  const sdsl::bit_vector& G_bits =
      matrixboss_plain.get_subset_rank_structure().G_bits;
  const sdsl::bit_vector& T_bits =
      matrixboss_plain.get_subset_rank_structure().T_bits;
  ...
  sbwt::write_log(
      "Space on disk: " +
          to_string(bytes_written * 8.0 /
                    matrixboss_plain.number_of_subsets()) +
          " bits per column, " +
          to_string(bytes_written * 8.0 / matrixboss_plain.number_of_kmers()) +
          " bits per k-mer",
      sbwt::LogLevel::MAJOR);
