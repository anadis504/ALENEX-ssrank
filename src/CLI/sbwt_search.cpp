...
int64_t run_queries_streaming(reader_t& reader, writer_t& writer,
                              const sbwt_t& sbwt) {
...
template <typename sbwt_t, typename reader_t, typename writer_t>
int64_t run_queries_not_streaming(reader_t& reader, writer_t& writer,
                                  const sbwt_t& sbwt) {
  int64_t total_micros = 0;
  int64_t number_of_queries = 0;
...
template <typename sbwt_t, typename reader_t, typename writer_t>
int64_t run_file(const string& infile, const string& outfile,
                 const sbwt_t& sbwt) {
  reader_t reader(infile);
  writer_t writer(outfile);
  if (sbwt.has_streaming_query_support()) {
    write_log("Running streaming queries from input file " + infile +
                  " to output file " + outfile,
              LogLevel::MAJOR);
    return run_queries_streaming<sbwt_t, reader_t, writer_t>(reader, writer,
                                                             sbwt);
  } else {
    write_log("Running non-streaming queries from input file " + infile +
                  " to output file " + outfile,
              LogLevel::MAJOR);
    return run_queries_not_streaming<sbwt_t, reader_t, writer_t>(reader, writer,
                                                                 sbwt);
  }
}
...
int64_t run_queries(const vector<string>& infiles,
                    const vector<string>& outfiles, const sbwt_t& sbwt,
                    bool gzip_output) {
  if (infiles.size() != outfiles.size()) {
    string count1 = to_string(infiles.size());
    string count2 = to_string(outfiles.size());
    throw std::runtime_error(
        "Number of input and output files does not match (" + count1 + " vs " +
        count2 + ")");
  }

  typedef seq_io::Reader<seq_io::Buffered_ifstream<seq_io::zstr::ifstream>>
      in_gzip;
  ...
  for (int64_t i = 0; i < infiles.size(); i++) {
    ...
    if (gzip_input && gzip_output) {
      n_queries_run +=
          run_file<sbwt_t, in_gzip, out_gzip>(infiles[i], outfiles[i], sbwt);
    }
    if (gzip_input && !gzip_output) {
      n_queries_run +=
          run_file<sbwt_t, in_gzip, out_no_gzip>(infiles[i], outfiles[i], sbwt);
    }
    if (!gzip_input && gzip_output) {
      n_queries_run +=
          run_file<sbwt_t, in_no_gzip, out_gzip>(infiles[i], outfiles[i], sbwt);
    }
    if (!gzip_input && !gzip_output) {
      n_queries_run += run_file<sbwt_t, in_no_gzip, out_no_gzip>(
          infiles[i], outfiles[i], sbwt);
    }
  }
...
int search_main(int argc, char** argv) {
  ...
  options.add_options()("o,out-file", "Output filename.",
      ...
      "q,query-file",
      "The query in FASTA or FASTQ format, possibly gzipped. Multi-line FASTQ "
      "is not supported. If the file extension is .txt, this is interpreted as "
      "a list of query files, one per line. In this case, --out-file is also "
      "interpreted as a list of output files in the same manner, one line for "
      "each input file.",
      cxxopts::value<string>())(
      "z,gzip-output",
  ...
  bool multi_file =
      queryfile.size() >= 4 && queryfile.substr(queryfile.size() - 4) == ".txt";
  if (multi_file) {
