...
# adding new variant (built from plain-matrix)
Add new `.hh`file to `./include/sbwt/` executing the needed methods (check some examples).
* modify `./include/sbwt/variants.hh`to include the new variant-file and define its `type`.
* modufy 4 files: `./src/CLI/sbwt_search.cpp`, `./src/CLI/sbwt_build.cpp`, (`./src/CLI/sbwt_ascii_export.cpp`, `./src/CLI/sbwt_build_from_plain_matrix.cpp`)
