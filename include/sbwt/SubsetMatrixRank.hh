...
class SubsetMatrixRank {
 public:
  ...
  rank_support_t* rank_supports[4];
  // Rank supports
  ...
  int64_t rank_by_charidx(int64_t pos, int64_t char_idx) const {
    return (*rank_supports[char_idx]).rank(pos);
  }
  ...
  SubsetMatrixRank(const sdsl::bit_vector& A_bits,
  ...
  SubsetMatrixRank& operator=(const SubsetMatrixRank& other) {
    if (&other != this) {
      ...
      this->rank_supports[0] = &(this->A_bits_rs);
      this->rank_supports[1] = &(this->C_bits_rs);
      this->rank_supports[2] = &(this->G_bits_rs);
      this->rank_supports[3] = &(this->T_bits_rs);
  ...
  int64_t serialize(ostream& os) const {
    ...
    write_log("MatrixRank bit vectors total " +
                  to_string((double)written / A_bits.size() * 8) +
                  " bits total per node",
              LogLevel::MINOR);
    return written;
  }
