...
class BlockedCorrectionSetsBase4Rank67 {
  uint64_t _logb = 6;
  ...
  uint64_t block_constant = 4;
  uint64_t CS_CONSTANT = 7;

 public:
  BlockedCorrectionSetsBase4Rank67() {};

  BlockedCorrectionSetsBase4Rank67(const std::string& seq,
    ...
    std::cout << "P array size: " << _p.size() * sizeof(uint32_t) << " bytes"
              << std::endl;
    std::cout << "_bits array size: " << _bits.size() * sizeof(uint32_t) << " bytes"
              << std::endl;         
