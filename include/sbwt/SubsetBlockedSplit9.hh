...
class SubsetBlockedSplitRank9 {
 ...
 public:
  ...
  SubsetBlockedSplitRank9(const SubsetBlockedSplitRank9& other) {
    assert(&other != this);  // What on earth are you trying to do?
    operator=(other);
  }

  SubsetBlockedSplitRank9& operator=(const SubsetBlockedSplitRank9& other) {
    if (&other != this) {
